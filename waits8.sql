/*
   Show some of the system statistics(V$SYSSTAT), waits statistics(V$SYSTEM_EVENT), file statistics(V$FILESTAT)
   from the startup of the instance
   Tested on: Oracle8
   Required:TIMED_STATISTICS=TRUE in INIT.ORA
*/
set term off
column inst noprint new_value inst
select 'waits_'||instance||'_'||host_name||'_'||to_char(sysdate,'dd_mm_rrrr_hh24_mi')||'.lst' as inst from v$thread,v$instance;
set term on
...
select file#,phyrds,phyblkrd,readtim,maxiortm,phywrts,phyblkwrt,writetim,maxiowtm from v$filestat;

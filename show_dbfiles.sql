--
--  Script : show_dbfiles.sql
--  Purpose: Show all dest parameters,datafiles,tempfiles,redo log files in multitenant databases (12c+)
--  Version: 12c,19c,23c
--
@save_sqp_set
...
WHERE
    ...
    AND trim(name) in ('ADR Home','Diag Trace','Diag Alert','Diag Incident','Diag Cdump','Health Monitor','Default Trace File')
union all
SELECT 'data file' as name,file_name as value FROM cdb_data_files
union all
SELECT 'temp file' as name,file_name as value FROM cdb_temp_files
union all
...
prompt by example :
prompt lsnrctl status LISTENER_cdb1|grep 'Listener Log File'
prompt lsnrctl status LISTENER_cdb1|grep 'Listener Parameter File'
prompt

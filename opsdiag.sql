...
-- ------------------------------------------------------------------------ 
-- PURPOSE: 
-- This script is intended to provide a user friendly guide to troubleshoot RAC or 
-- OPS hang or slow performance scenerios.  The script includes information to 
-- gather a variety of important debug information to determine the cause of an 
-- RAC or OPS hang.  The script will create a file called opsdiag_<timestamp>.out 
-- in your local directory while dumping systemstate dumps in the user_dump_dest(s)
-- and background_dump_dest(s) on all nodes if on 8.1.7 or above.
--
-- ------------------------------------------------------------------------ 
-- DISCLAIMER: 
...
spool opsdiag_&&dbname&&timestamp&&suffix
set lines 200
set pagesize 35
set trim on
set trims on
...
-- experiencing timeouts.  See the 'WAITING SESSIONS', 'PCM LOCK BLOCKERS', 
-- 'PCM LOCK WAITERS', and 'TOP 10 WAIT EVENTS ON SYSTEM' sections if the 
...
'single-task message', 'wakeup time manager')
...
-- PCM LOCKS NOT GRANTED FOR WAITING SESSIONS:
-- 
-- This query will find ocks that are currently not in a grant status for any 
-- sessions that are waiting.  This will show us what locks may be having problems
-- opening, converting, or cancelling.
-- due to contention.  
set numwidth 5
column lockstate format a16 tru;
column event format a25 tru;
...
-- DUMPING FULL LOCK INFORMATION PCM LOCKS NOT GRANTED FOR WAITING SESSIONS:
-- This may take a little while...
-- 
...
-- DUMPING FULL RESOURCE INFORMATION PCM LOCKS NOT GRANTED FOR WAITING SESSIONS:
-- This may take a little while...
...
-- ---------------------------------------------------------------------------
Prompt;
Prompt opsdiag output files have been written to:;
Prompt;
host pwd

...
connect sys/gongx1faca1@t1 as sysdba
set markup HTML ON entmap off
alter session set nls_date_format='HH24:Mi:SS MM/DD/YY';
set heading off
spool re.html
select 'STREAMS Health Check (V3.0.5) for '||global_name||' on Instance='||instance_name||' generated:'||sysdate o  from global_name, v$instance;
...
prompt Analysis: <a href="#History"> History </a>  <a href="#Rules"> Rules </a>  <a href="#Notification"> Notifications </a> <a href="#Configuration checks"> Configuration </a> <a href="#Performance Checks"> Performance </a>
...
COLUMN PURGEABLE HEADING 'Purgeable|ArchiveLog' format a9
...
user_procedure,
decode(error_handler,'Y','Error','N','DML','UNKNOWN') TYP, APPLY_Database_link DBLINK, apply_name, assemble_lobs
...
Select apply_name, source_database,source_commit_scn,message_number, message_count,
   local_transaction_id, error_message , error_creation_time
...
col BEGIN_INTERVAL_TIME HEADING 'Interval|Begin|Time'
col END_INTERVAL_TIME HEADING 'Interval|End|Time'
col INSTANCE_NUMBER HEADING 'Instance|Number'
prompt
prompt ++ Streams Buffered Queue History for last day ++

select s.begin_interval_time,s.end_interval_time , bq.* from dba_hist_buffered_queues bq, dba_hist_snapshot s where bq.snap_id=s.snap_id and s.end_interval_time >= systimestamp-1 order by bq.queue_schema,bq.queue_name,s.end_interval_time;


prompt
prompt ++ Streams Buffered Subscriber History for last day ++

select s.begin_interval_time,s.end_interval_time , bs.* from dba_hist_buffered_subscribers bs, dba_hist_snapshot s where bs.snap_id=s.snap_id and s.end_interval_time >= systimestamp-1 order by bs.queue_schema,bs.queue_name,bs.subscriber_id,s.end_interval_time;
...
select s.begin_interval_time,s.end_interval_time , sa.* from dba_hist_streams_apply_sum sa, dba_hist_snapshot s where sa.snap_id=s.snap_id and s.end_interval_time >= systimestamp-1 order by 
...
prompt
prompt  ++  Current Contents of the STREAMS Pool ++  
prompt   Applies only to versions 10.1.0.4+, and to this instance only
prompt

col comm HEAD 'Allocation Comment' format A18
col alloc_size HEAD 'Bytes Allocated' format 9999999999999999
...
column STATE_CHANGE_TIME HEADING 'Last|State Change|Time'
...
begin
  ...
  verify_init_parameter('aq_tm_processes', '0', TRUE, 
                        '+    This value should not be set explicitly in the init.ora or spfile.'||
                        'However if set, the preferred value is 1.', alert_if_not_found=>FALSE);
